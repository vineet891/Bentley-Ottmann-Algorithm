...
void StatusTree::initNull(NodePtr n, NodePtr p) {
    n->data = Segment();
    n->parent = p;
    n->left = nullptr;
    n->right = nullptr;
...
// Print Functions
void StatusTree::printNode(NodePtr node) {
    cout << "Node - ";
    node->data.printSegment();
}

void StatusTree::preOrderHelper(NodePtr root) {
    if(root != endNull) {
        this->printNode(root);
...
void StatusTree::postOrderHelper(NodePtr root) {
    if(root != endNull) {
...
void StatusTree::inOrderHelper(NodePtr root) {
    if(root != endNull) {
...
void StatusTree::rightRotate(NodePtr node) {
...
void StatusTree::leftRotate(NodePtr node) {
...
void StatusTree::insertHelper(NodePtr node) {
    NodePtr temp;
...
void StatusTree::insertTree(Segment s) {
    Node *node = new Node();
    node->parent = nullptr;
    node->data = s;
    node->left = endNull;
...
// Search Function
NodePtr StatusTree::searchHelper(NodePtr root, Segment key) {
...
NodePtr StatusTree::searchTree(Segment s) {
